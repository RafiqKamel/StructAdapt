...
def print_simplified(graph_triples, v2c):
    ...
    concept_stack_depth = {
        None: 0
    }  # size of the stack when the :instance-of triple was encountered for the variable
...
def main(args):
    ...
    if args.triples_output is not None:
        ...
        reentrances_file = open(
            args.triples_output.replace("graph", "reentrances"), "w"
        )
        ...
        for idx, (amr, surf) in enumerate(zip(amrs, surfs)):
            ...
            if args.mode == "LINE_GRAPH":
                ...
                if instance_true:
                    count_instance += 1
                try:
                    nodes, triples = get_line_graph_new(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Preprocess AMR into linearised forms with multiple preprocessing steps (based on Konstas et al. ACL 2017)"
    )
    ...
    parser.add_argument(
        "output", type=str, help="output file, either AMR or concept list"
    )
    ...
    parser.add_argument(
        "--scope", action="store_true", help="add scope markers to graph"
    )
    parser.add_argument(
        "--add-reverse",
        action="store_true",
        help="whether to add reverse edges in the graph output",
    )
    parser.add_argument(
        "--triples-output", type=str, default=None, help="triples output for graph2seq"
    )
    parser.add_argument(
        "--map-output",
        type=str,
        default=None,
        help="mapping output file, if using anonymisation",
    )
    parser.add_argument(
        "--anon-surface",
        type=str,
        default=None,
        help="anonymized surface output file, if using anonymisation",
    )
    parser.add_argument(
        "--nodes-scope",
        type=str,
        default=None,
        help="anonymized AMR graph file, with scope marking, used in baseline seq2seq models",
    )

    args = parser.parse_args()

    assert (args.triples_output is not None) or (
        args.mode != "GRAPH"
    ), "Need triples output for graph mode"
    assert (args.map_output is not None) or (
        not args.anon
    ), "Need map output for anon mode"
    assert (args.anon_surface is not None) or (
        not args.anon
    ), "Need anonymized surface output for anon mode"
