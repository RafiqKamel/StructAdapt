...
class Seq2SeqDataset(AbstractSeq2SeqDataset):
    ...
    def collate_fn(self, batch):
        ...
        # batch_encoding["graphs"] = torch.tensor([], dtype=torch.long)
        # print(batch_encoding.keys())
        # print(batch_encoding["input_ids"].size())
        # print(batch_encoding["ids"].size())
        # print(len(batch_encoding["graphs"]))
...
class Seq2SeqDataCollator:
    ...
    def _encode(self, batch) -> Dict[str, torch.Tensor]:
        batch_encoding = self.tokenizer.prepare_seq2seq_batch(
            ...
            padding=(
                "max_length" if self.tpu_num_cores is not None else "longest"
            ),  # TPU hack
            return_tensors="pt",
            **self.dataset_kwargs,
        )
